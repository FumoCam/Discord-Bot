...
class VoiceChannelHoist(commands.Cog):
    def __init__(self, bot: BotClass):
        ...
        # Only enable if explicitly specified
        if not self.bot.CFG.get("voice_hoist_when_active", False):
            print(
                "['voice_hoist_when_active' not set to 'true', disabling voice channel hoist subroutine]"
            )
            return
